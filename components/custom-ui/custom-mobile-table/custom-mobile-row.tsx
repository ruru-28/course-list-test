'use client';
// components/custom-ui/custom-mobile-table/custom-mobile-row.tsx
...
export interface CustomMobileRowProps {
...
export function CustomMobileRow({
  id,
  fields,
  dragHandle,
  checkbox,
  ...
  return (
    ...
    >
      ...
      <div
        ref={headerRef}
        className={cn(
          'bg-[#e5e7eb] border-b border-[#e5e7eb] border-l-0 border-r-0 border-t-0 border-solid flex flex-wrap items-center justify-between px-4 py-[8px] min-h-[41px] w-full',
      ...
      >
        <div className="flex rounded-sm items-center shrink-0">
          {/* Drag Handle */}
          {dragHandle && (
            <div className="flex items-center p-[6px]" {...dragHandle.listeners}>
          ...
          <p
            className={cn(
              `font-semibold text-sm ${checkbox ? 'mx-2' : 'mr-2'}`,
          ...
          {/* Switch */}
          {switchProps && (
            <div className="flex items-center pr-[16px] py-0">
              <Switch
        ...
        {(actions || customActionMenu) && (
          <div 
            ...
            className={cn(
              ...
              hasWrapped && "w-full pt-2 gap-2" // Full width when wrapped to ensure proper alignment
            )}
          >
      ...
      {fields.map((field, index) => (
        <div
          key={index}
          className={cn(
            'border-b border-[#e5e7eb] border-l-0 border-r-0 border-t-0 border-solid last:border-b-0 flex items-start justify-between px-4 py-[8px] w-full shrink-0',
            fieldRowClassName
          )}
        >
          <div className="flex-[1_0_0] gap-[8px] grid grid-cols-[minmax(0,_1fr)_minmax(0,_2fr)] grid-rows-[repeat(1,_fit-content(100%))] min-h-px min-w-px relative">
            ...
            {/* Field Value */}
            <div className="col-[2] flex items-start justify-self-stretch self-start text-sm">
              ...
              typeof field.value === 'number' ? (
                ...
                <div className={cn('w-full text-sm', disabled && 'opacity-60')}>
                  {field.value}
                </div>
              )}
            </div>
